...
class ROIExtractor:
    ...
    def extract(self, frame: np.ndarray, face_bbox: tuple[int, int, int, int]) -> np.ndarray | None:
        """
    ...
    def draw_roi(
        ...
        # Add label
        cv2.putText(
            frame_copy, self.region.upper(), (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1
        )
